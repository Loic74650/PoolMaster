...
//Calibration offsets
float PhValueOffset = 0.93f;
float OrpValueOffset = -185.0f;
...
int eePhValueOffset_Address = eeOrp_RegulationOnOff_Address + sizeof(byte);
int eeOrpValueOffset_Address = eePhValueOffset_Address + sizeof(float);
int eePh_SetPoint_Address = eeOrpValueOffset_Address + sizeof(float);
...
void messageReceived(String &topic, String &payload) 
{
  ...
  if(topic == TmpStrPool)
  {
      ...
      else
      {
        ...
        if(command.containsKey(F("PhOffset")))//"PhOffset" command which sets the calibration offset for Ph
        {          
           PhValueOffset = (float)command[F("PhOffset")];
           EEPROM.put( eePhValueOffset_Address, PhValueOffset);
        }       
        else 
        if(command.containsKey(F("OrpOffset")))//"OrpOffset" command which sets the calibration offset for ORP
        {          
           OrpValueOffset = (float)command[F("OrpOffset")];
           EEPROM.put( eeOrpValueOffset_Address, OrpValueOffset);
        }
        else  
...
void getMeasures(DeviceAddress deviceAddress_0)
{
  ...
  //Ph
  double ph_sensor_value = analogRead(PH_MEASURE)* 5000.0 / 1023.0 / 1000.0;                          // from 0.0 to 5.0 V
  PhValue = PhValueOffset + 7.0 - ((2.5 - ph_sensor_value)/(0.257179 + 0.000941468 * TempValue));     // formula to compute pH which takes water temperature into account
  //PhValue = (0.0178 * ph_sensor_value * 200.0) - 1.889;                                             // formula to compute pH
  ...
  Serial<<F("PhValue: ")<<PhValue<<F(" - ")<<_endl;

  //ORP
  float orp_sensor_value = analogRead(ORP_MEASURE) * 5000.0 / 1023.0 / 1000.0;                      // from 0.0 to 5.0 V
  OrpValue = OrpValueOffset + ((2.5 - orp_sensor_value) / 1.037) * 1000.0;                          // from -2000 to 2000 mV where the positive values are for oxidizers and the negative values are for reducers
  ...
  Serial<<F("OrpValue: ")<<OrpValue<<F("mV")<<_endl;  
}
...
void RestoreVariables()
{
    ...
    EEPROM.get( eePhValueOffset_Address, fVar); if((fVar>-5.0f) && (fVar<5.0f)) PhValueOffset = fVar; else EEPROM.put( eePhValueOffset_Address, PhValueOffset); Serial<<F("PhValueOffset: ")<<PhValueOffset<<_endl;
    EEPROM.get( eeOrpValueOffset_Address, fVar); if((fVar>-1000.0f) && (fVar<1000.0f)) OrpValueOffset = fVar; else EEPROM.put( eeOrpValueOffset_Address, OrpValueOffset); Serial<<F("OrpValueOffset: ")<<OrpValueOffset<<_endl;
                  ...
                  // send a standard http response header
                    ...
                    {  
                        ...
                        client<<F("<html>")<<_endl;
                                        ...
                                        client<<F("if (this.readyState == 4) {")<<_endl;
                                            client<<F("if (this.status == 200) {")<<_endl;
                                                client<<F("if (this.responseXML != null) {")<<_endl;
                                                    ...
                                                    client<<F("document.getElementById(\"pHCal\").innerHTML =")<<_endl;
                                                    client<<F("this.responseXML.getElementsByTagName('CalibOffset')[0].childNodes[0].nodeValue;")<<_endl;
                                                    client<<F("document.getElementById(\"ORPCal\").innerHTML =")<<_endl;
                                                    client<<F("this.responseXML.getElementsByTagName('CalibOffset')[1].childNodes[0].nodeValue;")<<_endl;
                            ...
                            client<<F("</tr>")<<_endl;  
                            client<<F("<tr>")<<_endl;
                            client<<F("<td>Calib. offset</td>")<<_endl;
                            client<<F("<td><span id=\"pHCal\">...</span></td>")<<_endl;
                            client<<F("<td><span id=\"ORPCal\">...</span></td>")<<_endl;
                            client<<F("</tr>")<<_endl;                         
                            client<<F("<tr>")<<_endl;
